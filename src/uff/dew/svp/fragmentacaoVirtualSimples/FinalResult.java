...
public class FinalResult {
	...
	if ( q.getAggregateFunctions() != null && q.getAggregateFunctions().size() > 0 ) { // possui funcoes de agregacao na clausula LET.
			...
			+	" let $c:= collection('" + AVPConst.TEMP_COLLECTION_NAME + "')/partialResult/" + sbq.getConstructorElement().replaceAll("[</>]", "") + "\r\n"
			//+	" let $c:= collection('" + FinalResultComposer.getTempCollectionName() + "')/partialResult/" + sbq.getConstructorElement().replaceAll("[</>]", "") + "\r\n"
...
//					}
			...
			finalResultXquery = finalResultXquery + "\r\n\t" + sbq.elementAfterConstructor.replace("<", "</") + " } " + sbq.getConstructorElement().replace("<", "</");
...
//			finalResultXquery = sbq.getConstructorElement() + " { \r\n " 
//			  + " for $ret in collection('" + AVPConst.TEMP_COLLECTION_NAME + "')/partialResult/" 
...
//	          + sbq.getConstructorElement().replace("<", "</");  
			finalResultXquery = sbq.getConstructorElement() + " { \r\n " 
					  + " for $ret in collection('" + AVPConst.TEMP_COLLECTION_NAME + "')/partialResult/orderby/" 
					  //+ " for $ret in collection('" + FinalResultComposer.getTempCollectionName() + "')/partialResult/orderby/" 
					  + sbq.getConstructorElement().replaceAll("[</>]", "") + "/" + sbq.getElementAfterConstructor().replaceAll("[</>]", "")					          
			          + "\r\n order by " + orderByClause + (q.getOrderByType()!=null && !q.getOrderByType().equals("")? " " + q.getOrderByType(): " ascending")
			          + "\r\n return $ret/element"
			          + "\r\n } "
			...
			finalResultXquery = sbq.getConstructorElement() + " { \r\n "
							  +	" for $ret in collection('" + AVPConst.TEMP_COLLECTION_NAME + "')/partialResult/" + sbq.getConstructorElement().replaceAll("[</>]", "")
							  //+	" for $ret in collection('" + FinalResultComposer.getTempCollectionName() + "')/partialResult/" + sbq.getConstructorElement().replaceAll("[</>]", "")
		...
		String finalResult = ExecucaoConsulta.executeQuery(finalResultXquery);

...
public class ExistsJoinOperation {
	...
	public void verifyInputQuery() {
		...
		int pos = -1;
		String tmp = "";
		int numberForClauses = 0; // numero de clausulas FOR		

		pos = this.getXquery().toUpperCase().indexOf("FOR $"); // posicao da primeira clausula FOR.
		...
		while ( pos >= 0 ) {		
			...
			//pos = this.getXquery().toUpperCase().indexOf("FOR $"); // posicao da primeira clausula DOC().						
			tmp = this.getXquery().substring(pos+5, this.getXquery().length()); // Obtem a string apos a definicao de for.

			this.setXquery(tmp);
			pos = this.getXquery().toUpperCase().indexOf("FOR $"); // posicao da proxima clausula FOR, se houver.		
	...
	/* Recebe como parametros os caminhos dos join */
	//Chamado por FLWOR.insertJoinPredicate()
	public void verifyJoins(String pathJoin1, String pathJoin2, String varJoin1, String varJoin2, String atrJoin1, String atrJoin2) throws IOException {		
		...
		if (forClauses.containsKey("$"+varJoin1)){ // verifica se a variavel das juncoes j existem na hashtable, de forma a substituir o nome da varivel pelo caminho que ela representa.			
			pathJoin1 = pathJoin1.replace("$"+varJoin1, q.getPathVariable("$"+varJoin1));
		}

		if (forClauses.containsKey("$"+varJoin2)){ // verifica se a variavel das juncoes j existem na hashtable, de forma a substituir o nome da varivel pelo caminho que ela representa.			
		...
		if (!q.getPathVariable("$"+varJoin1).equals("") && !q.getPathVariable("$"+varJoin2).equals("")) {
			...
			if ( cardinality > 0 && cardinality2 > 0) { // especificou o caminho completo
				// Verificar se o atributo que possui a menor cardinalidade, uma vez que as chaves primarias, em geral, possuem menor 
				// cardinalidade que as chaves estrangeiras.

				//if (cardinality < cardinality2) {
				//Alterado por Luiz Matos, considerando o join de maior cardinalidade, evitando assim
				//o problema de usar um atributo de fragmentacao cuja cardinalidade eh pequena para determinado numero de nodes de processamento
				if (cardinality > cardinality2) {
				...
				if (cardinality > 1) { // so pode fragmentar se houver relao 1:N.				
				...
				if (cardinality2 > 1) { // so pode fragmentar se houver relao 1:N.

...
public class XQueryEngine {
	...
	protected void processSimpleNode(final SimpleNode node, final boolean debug) 
		...
		if (element == "ElmtConstructor"){
			...
			// Compilacao e execucao do FLWOR
			...
			//if ("ElmtConstructor".equals(node)) {
//				int aux = 0;
//				byte[] pog = flwor.toString().getBytes();
//				String decodedDataUsingUTF8 = new String(pog, "UTF-8"); 
				//do {
				   // String tag = pog.replace("\n\r", " ");

//				    String tag = decodedDataUsingUTF8.substring(decodedDataUsingUTF8.toString().indexOf("refAPT: null - label: ")+7, pog.toString().indexOf("--"));
					//System.out.println("veja isso = " + decodedDataUsingUTF8);
					//decodedDataUsingUTF8 = decodedDataUsingUTF8.substring(decodedDataUsingUTF8.toString().indexOf("-"));
					//aux = decodedDataUsingUTF8.toString().indexOf(tag);
				//}	while(aux < tmp.toString().length());
			//}

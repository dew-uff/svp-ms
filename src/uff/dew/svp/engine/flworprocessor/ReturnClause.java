	...
	protected void processSimpleNode(SimpleNode node, boolean debug){
		...
		if (element.equals("ElmtConstructor")){
			...
			if (expr.getOperator().getName().equals("Select"))
				...
				//System.out.println("_functionOperators = " + expr.getOperator().getApt().getAptNode().getRootNode().getLabel());
			...
			if (q.isWaitingXpathAggregateFunction()) {
				...
				if ( q.getLastReturnVariable()!=null && q.getLastReturnVariable().equals(node.getText()) ){
					// esta fechando o elemento que referencia funcao de agregacao. Ex.: <total_items>count($l)</total_items>
					...
					if ( variableName != null && hashTmp.get(variableName) != null && hashTmp.get(variableName).indexOf("null:") == -1) {
						...
						//System.out.println("functionClause = " + functionClause);
						Hashtable<String, String> aggTmp = q.getAggregateFunctions();
						Hashtable<String, String> aggTmp2 = q.getAggregateReturn();
						//System.out.println("agora vaiiii ");


						//this.getFunctionOperatorsList().get(0).getName();
//						for (int i=0; i<q.getAggregateReturn().size(); i++) {
//							System.out.println("key - " + q.getAggregateReturn().keys().nextElement());
//
//							System.out.println("value - " + q.getAggregateReturn().get(q.getAggregateReturn().keys().nextElement()));
//
//						}
					//	System.out.println("q.getAggregateFunctions().size() = " + q.getAggregateFunctions().size());
//						for (int i=0; i<q.getAggregateFunctions().size(); i++) {
//							System.out.println("key 2 - " + q.getAggregateFunctions().keys().nextElement());
//
//							System.out.println("value 2 - " + q.getAggregateFunctions().get(q.getAggregateFunctions().keys().nextElement()));
//						}


						String tmp = aggTmp.get(functionClause);	
						//String tmp2 = aggTmp2.get(q.getLastReadLetVariable());

//						System.out.println("q.getAggregateFunctions().size() = " + q.getAggregateFunctions().size());
//						System.out.println("q.getAggregateReturn().size() = " + q.getAggregateReturn().size());

						//System.out.println("q.getLastReadLetVariable() = " + q.getLastReadLetVariable());
						//System.out.println("q.getXpathAggregateFunction() = " + q.getXpathAggregateFunction());
						//	System.out.println("bug = " + tmp.replace(q.getLastReadLetVariable(), q.getXpathAggregateFunction()));

						//System.out.println(tmp.replace(q.getLastReadLetVariable(), q.getXpathAggregateFunction()));
		...
		if (processChild & (node.jjtGetNumChildren()>0)){
			for (int i=0; i<node.jjtGetNumChildren(); i++){
				//System.out.println("ta aqui - " + node.jjtGetChild(i).toString());
	...
	protected void addSelectOperator(AbstractOperator operator){
		if (this._selectOperators.size()==0)
			...
			for (int i=0; i<this._selectOperators.size(); i++){
				...
				// Comparacao dos nodos root das arvores

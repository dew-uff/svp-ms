...
public class ForLetClause extends Clause {
	...
	public void compileForLet(SimpleNode node, boolean debug){	
		...
		if (!forClauses.containsKey(q.getFragmentationVariable())){ // verifica se a varivel ja existe na hashTable
	...
	protected void processSimpleNode(SimpleNode node, boolean debug){
		...
		if ("ForClause".equals(element)){					
			...
			q.setFragmentationVariable("$"+newNode.getLabel()); // indica a ultima varivel XML lida, referente a um FOR.	
		}

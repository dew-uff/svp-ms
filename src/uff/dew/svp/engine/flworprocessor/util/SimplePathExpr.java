		...
		if (element == "Slash"){
			...
			try {
				...
				if (!q.isElementConstructor() && !q.isAddedPredicate() && 
					...
					String orderBy = "";
					//Luiz Matos - adiciona parenteses para fechar a função de agregação utilizada no order by
				...
				//Luiz Matos - adiciona as variaveis utilizadas no group by
				else if (!q.isElementConstructor() && q.isGroupByClause()) {
					String groupBy = "";

					groupBy = q.getGroupBy() + (q.getGroupBy().equals("")? node.getText():"/"+node.getText());

             		q.setGroupBy(groupBy); // elementos que serao utilizados para o agrupamento do resultado final, alem de auxiliar na identificacao dos LET que devem constar na consulta final.
				}

...
public class NqpAllocator {
	 ...
	 */
	public synchronized void allocateNQPsForQueryProcessing(int[] allocatedNQP_id) {
		//int idGQT;

		if (allocatedNQP_id.length > a_nqp.length)
			throw new IllegalArgumentException("NQP_Allocator Exception: " + allocatedNQP_id.length + " were demanded but only "	+ a_nqp.length + " are available!");
...
//		idGQT = loadInfo[0].getNQP_Id();
		...
		notifyAll();
		//System.out.println("idGQT: " + idGQT);
		//return idGQT;
	}
	...
	//by Luiz Matos
	public synchronized void disposeNQPs(int[] allocatedNQP_id) throws RemoteException, MalformedURLException, NotBoundException {
		NqpLoadInfo loadInfo;
		...
		for (int i = 0; i < allocatedNQP_id.length; i++) {
			loadInfo = a_loadInfo[allocatedNQP_id[i]];
			if (!a_loadOrderedSet.remove(loadInfo))
				throw new IllegalStateException("NQP_Allocator.disposeNQPs(): loadInfo not found in the ordered set - "	+ loadInfo);
			loadInfo.decreaseLoad(LOAD_LQT);
			a_loadOrderedSet.add(loadInfo);
			//loadInfo.getNQP().shutdown();
		}
